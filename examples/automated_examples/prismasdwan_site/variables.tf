...
variable "sgi_config" {
  ...
  validation {
    condition = var.sgi_config == null || (
      var.sgi_config.sgi_vendor_id == "SGI_VENDOR_ID_TYPE_8909" &&
      var.sgi_config.sgi_tag >= 1 && var.sgi_config.sgi_tag <= 65533
    )
    error_message = "SGI vendor ID must be 'SGI_VENDOR_ID_TYPE_8909' and SGI tag must be between 1 and 65533."
  }
}
