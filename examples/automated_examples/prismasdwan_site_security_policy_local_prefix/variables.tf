...
variable "site_name" {
  description = "Name of the site where the security policy local prefix will be configured"
  type        = string
...
# Security Policy Prefix Configuration (Required)
variable "prefix_id" {
  description = "Prefix ID for the security policy local prefix (required)"
  type        = string
  default     = "security-policy-local-prefix-001"
...
variable "ipv4_prefixes" {
  description = "List of IPv4 prefixes in CIDR notation for security policy enforcement (e.g., 192.168.1.0/24)"
  type        = list(string)
  default     = [
    "192.168.20.0/24",
    "192.168.21.0/24",
...
variable "ipv6_prefixes" {
  description = "List of IPv6 prefixes in CIDR notation for security policy enforcement (e.g., 2001:db8::/64)"
  type        = list(string)
  default     = [
    "2001:db8:4000::/64",
    "2001:db8:5000::/64",
...
variable "tags" {
  description = "Tags for the site security policy local prefix (max 10 tags, max 128 chars each)"
  type        = list(string)
  default     = ["terraform", "security", "ngfw", "local-prefix", "site-specific", "firewall"]
