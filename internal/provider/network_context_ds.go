...
type dsModelWithFilterNetworkContext struct {
	...
	Items        []*dsModelNetworkContextScreen `tfsdk:"items"`
}
...
func (d *networkContextDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterNetworkContext
	...
	get_path := "/sdwan/v2.0/api/networkcontexts/{network_context_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_network_context", (*list_request.ResponseErr).Error())
		}
		return
	}
