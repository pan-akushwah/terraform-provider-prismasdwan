...
// Data source.
var (
	_ datasource.DataSource              = &vrfContextDataSource{}
	_ datasource.DataSourceWithConfigure = &vrfContextDataSource{}
)
...
//	     }
//	  }
func NewVrfContextDataSource() datasource.DataSource {
	return &vrfContextDataSource{}
}

type vrfContextDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterVrfContext struct {
	...
	Items        []*dsModelVRFContextScreen `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *vrfContextDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_vrf_context"
}

// Schema defines the schema for this data source.
func (d *vrfContextDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *vrfContextDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *vrfContextDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterVrfContext
	...
	get_path := "/sdwan/v2.0/api/vrfcontexts/{vrf_context_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_vrf_context", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelVRFContextScreen
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to VRFContextScreen", json_err.Error())
			return
		}
