...
// Data source.
var (
	_ datasource.DataSource              = &globalPrefixFilterDataSource{}
	_ datasource.DataSourceWithConfigure = &globalPrefixFilterDataSource{}
)
...
//	     }
//	  }
func NewGlobalPrefixFilterDataSource() datasource.DataSource {
	return &globalPrefixFilterDataSource{}
}

type globalPrefixFilterDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterGlobalPrefixFilter struct {
	...
	Items        []*dsModelGlobalPrefixFilterScreen `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *globalPrefixFilterDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_global_prefix_filter"
}

// Schema defines the schema for this data source.
func (d *globalPrefixFilterDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *globalPrefixFilterDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *globalPrefixFilterDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterGlobalPrefixFilter
	...
	get_path := "/sdwan/v2.0/api/globalprefixfilters/{filter_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_global_prefix_filter", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelGlobalPrefixFilterScreen
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to GlobalPrefixFilterScreen", json_err.Error())
			return
		}
