...
// Data source.
var (
	_ datasource.DataSource              = &elementRoutingAspathaccesslistDataSource{}
	_ datasource.DataSourceWithConfigure = &elementRoutingAspathaccesslistDataSource{}
)
...
//	     }
//	  }
func NewElementRoutingAspathaccesslistDataSource() datasource.DataSource {
	return &elementRoutingAspathaccesslistDataSource{}
}

type elementRoutingAspathaccesslistDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterElementRoutingAspathaccesslist struct {
	...
	Items        []*dsModelRoutingAccessListScreenV2N1 `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *elementRoutingAspathaccesslistDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_element_routing_aspathaccesslist"
}

// Schema defines the schema for this data source.
func (d *elementRoutingAspathaccesslistDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
			...
			"items": dsschema.ListNestedAttribute{
				...
				NestedObject: dsschema.NestedAttributeObject{
					Attributes: map[string]dsschema.Attribute{
						...
						"as_path_regex_list": dsschema.ListNestedAttribute{
							Required:  false,
							Computed:  false,
							Optional:  true,
							Sensitive: false,
							NestedObject: dsschema.NestedAttributeObject{
								Attributes: map[string]dsschema.Attribute{
									...
									"as_path_regex": dsschema.StringAttribute{
										Required:  false,
										Computed:  false,
										Optional:  true,
										Sensitive: false,
...
func (d *elementRoutingAspathaccesslistDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *elementRoutingAspathaccesslistDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterElementRoutingAspathaccesslist
	...
	get_path := "/sdwan/v2.1/api/sites/{site_id}/elements/{element_id}/routing_aspathaccesslists/{config_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_element_routing_aspathaccesslist", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelRoutingAccessListScreenV2N1
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to RoutingAccessListScreenV2N1", json_err.Error())
			return
		}
		...
		if ans.AsPathRegexList == nil {
			...
			state.AsPathRegexList = []dsModelASPathRegex{}
		} else {
			state.AsPathRegexList = make([]dsModelASPathRegex, 0, len(ans.AsPathRegexList))
