...
type dsModelWithFilterNatZone struct {
	...
	Items        []*dsModelNATPolicyZone `tfsdk:"items"`
}
...
func (d *natZoneDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterNatZone
	...
	get_path := "/sdwan/v2.0/api/natzones/{nat_policy_zone_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_nat_zone", (*list_request.ResponseErr).Error())
		}
		return
	}
