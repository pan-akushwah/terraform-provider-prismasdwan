...
// Data source.
var (
	_ datasource.DataSource              = &elementMulticastRpDataSource{}
	_ datasource.DataSourceWithConfigure = &elementMulticastRpDataSource{}
)
...
//	     }
//	  }
func NewElementMulticastRpDataSource() datasource.DataSource {
	return &elementMulticastRpDataSource{}
}

type elementMulticastRpDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterElementMulticastRp struct {
	...
	Items        []*dsModelMulticastRPConfigScreen `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *elementMulticastRpDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_element_multicast_rp"
}

// Schema defines the schema for this data source.
func (d *elementMulticastRpDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
			...
			"items": dsschema.ListNestedAttribute{
				...
				NestedObject: dsschema.NestedAttributeObject{
					Attributes: map[string]dsschema.Attribute{
						...
						"groups": dsschema.ListNestedAttribute{
							Required:  false,
							Computed:  false,
							Optional:  true,
							Sensitive: false,
...
func (d *elementMulticastRpDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *elementMulticastRpDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterElementMulticastRp
	...
	get_path := "/sdwan/v2.0/api/sites/{site_id}/elements/{element_id}/multicastrps/{config_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_element_multicast_rp", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelMulticastRPConfigScreen
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to MulticastRPConfigScreen", json_err.Error())
			return
		}
		...
		if ans.Groups == nil {
			...
			state.Groups = []dsModelGroup{}
		} else {
			state.Groups = make([]dsModelGroup, 0, len(ans.Groups))
