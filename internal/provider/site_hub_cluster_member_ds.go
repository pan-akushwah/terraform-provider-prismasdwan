...
type dsModelWithFilterSiteHubClusterMember struct {
	...
	Items        []*dsModelHubClusterMemberScreen `tfsdk:"items"`
}
...
func (d *siteHubClusterMemberDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterSiteHubClusterMember
	...
	get_path := "/sdwan/v3.0/api/sites/{site_id}/hubclusters/{hub_cluster_id}/hubclustermembers/{hub_cluster_member_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_site_hub_cluster_member", (*list_request.ResponseErr).Error())
		}
		return
	}
