...
type dsModelWithFilterSiteLanNetwork struct {
	...
	Items        []*dsModelLANNetworkScreenV3N3 `tfsdk:"items"`
}
...
func (d *siteLanNetworkDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterSiteLanNetwork
	...
	get_path := "/sdwan/v3.3/api/sites/{site_id}/lannetworks/{lannetwork_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_site_lan_network", (*list_request.ResponseErr).Error())
		}
		return
	}
