...
// Data source.
var (
	_ datasource.DataSource              = &securityPolicyLocalPrefixDataSource{}
	_ datasource.DataSourceWithConfigure = &securityPolicyLocalPrefixDataSource{}
)
...
//	     }
//	  }
func NewSecurityPolicyLocalPrefixDataSource() datasource.DataSource {
	return &securityPolicyLocalPrefixDataSource{}
}

type securityPolicyLocalPrefixDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterSecurityPolicyLocalPrefix struct {
	...
	Items        []*dsModelSecurityPolicyV2LocalPrefixScreen `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *securityPolicyLocalPrefixDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_security_policy_local_prefix"
}

// Schema defines the schema for this data source.
func (d *securityPolicyLocalPrefixDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *securityPolicyLocalPrefixDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *securityPolicyLocalPrefixDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterSecurityPolicyLocalPrefix
	...
	get_path := "/sdwan/v2.0/api/ngfwsecuritypolicylocalprefixes/{prefix_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_security_policy_local_prefix", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelSecurityPolicyV2LocalPrefixScreen
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to SecurityPolicyV2LocalPrefixScreen", json_err.Error())
			return
		}

		// lets copy all items into state schema=SecurityPolicyV2LocalPrefixScreen
