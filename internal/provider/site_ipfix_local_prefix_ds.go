...
// Data source.
var (
	_ datasource.DataSource              = &siteIpfixLocalPrefixDataSource{}
	_ datasource.DataSourceWithConfigure = &siteIpfixLocalPrefixDataSource{}
)
...
//	     }
//	  }
func NewSiteIpfixLocalPrefixDataSource() datasource.DataSource {
	return &siteIpfixLocalPrefixDataSource{}
}

type siteIpfixLocalPrefixDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterSiteIpfixLocalPrefix struct {
	...
	Items        []*dsModelSiteIPFixPrefixAssociationScreen `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *siteIpfixLocalPrefixDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (d *siteIpfixLocalPrefixDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *siteIpfixLocalPrefixDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterSiteIpfixLocalPrefix
	...
	get_path := "/sdwan/v2.0/api/sites/{site_id}/ipfixlocalprefixes/{prefix_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_site_ipfix_local_prefix", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelSiteIPFixPrefixAssociationScreen
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to SiteIPFixPrefixAssociationScreen", json_err.Error())
			return
		}

		// lets copy all items into state schema=SiteIPFixPrefixAssociationScreen
