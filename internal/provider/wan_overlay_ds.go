...
// Data source.
var (
	_ datasource.DataSource              = &wanOverlayDataSource{}
	_ datasource.DataSourceWithConfigure = &wanOverlayDataSource{}
)
...
//	     }
//	  }
func NewWanOverlayDataSource() datasource.DataSource {
	return &wanOverlayDataSource{}
}

type wanOverlayDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterWanOverlay struct {
	...
	Items        []*dsModelWanOverlay `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *wanOverlayDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_wan_overlay"
}

// Schema defines the schema for this data source.
func (d *wanOverlayDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *wanOverlayDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *wanOverlayDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterWanOverlay
	...
	get_path := "/sdwan/v2.0/api/wanoverlays/{wan_overlay_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_wan_overlay", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to WanOverlay", json_err.Error())
			return
		}

		// lets copy all items into state schema=WanOverlay
