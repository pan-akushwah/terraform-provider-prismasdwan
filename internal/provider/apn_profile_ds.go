...
// Data source.
var (
	_ datasource.DataSource              = &apnProfileDataSource{}
	_ datasource.DataSourceWithConfigure = &apnProfileDataSource{}
)
...
//	     }
//	  }
func NewApnProfileDataSource() datasource.DataSource {
	return &apnProfileDataSource{}
}

type apnProfileDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterApnProfile struct {
	...
	Items        []*dsModelAPNProfileScreen `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *apnProfileDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_apn_profile"
}

// Schema defines the schema for this data source.
func (d *apnProfileDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *apnProfileDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *apnProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterApnProfile
	...
	get_path := "/sdwan/v2.0/api/apnprofiles/{apnprofile_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_apn_profile", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelAPNProfileScreen
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to APNProfileScreen", json_err.Error())
			return
		}
