...
type dsModelWithFilterElementNtp struct {
	...
	Items        []*dsModelElementNTPV2N1 `tfsdk:"items"`
}
...
func (d *elementNtpDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterElementNtp
	...
	get_path := "/sdwan/v2.1/api/elements/{element_id}/ntp/{ntp_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_element_ntp", (*list_request.ResponseErr).Error())
		}
		return
	}
