...
type dsModelWithFilterElementRoutingIpcommunitylist struct {
	...
	Items        []*dsModelRoutingCommunityListScreen `tfsdk:"items"`
}
...
func (d *elementRoutingIpcommunitylistDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterElementRoutingIpcommunitylist
	...
	get_path := "/sdwan/v2.0/api/sites/{site_id}/elements/{element_id}/routing_ipcommunitylists/{config_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_element_routing_ipcommunitylist", (*list_request.ResponseErr).Error())
		}
		return
	}
