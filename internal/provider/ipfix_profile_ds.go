...
type dsModelWithFilterIpfixProfile struct {
	...
	Items        []*dsModelIPFixProfileScreen `tfsdk:"items"`
}
...
func (d *ipfixProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterIpfixProfile
	...
	get_path := "/sdwan/v2.0/api/ipfixprofiles/{profile_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_ipfix_profile", (*list_request.ResponseErr).Error())
		}
		return
	}
