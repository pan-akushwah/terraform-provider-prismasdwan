...
type dsModelWithFilterElementSyslogServer struct {
	...
	Items        []*dsModelSyslogServerScreenV2N3 `tfsdk:"items"`
}
...
func (d *elementSyslogServerDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterElementSyslogServer
	...
	get_path := "/sdwan/v2.3/api/sites/{site_id}/elements/{element_id}/syslogservers/{syslogserver_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_element_syslog_server", (*list_request.ResponseErr).Error())
		}
		return
	}
