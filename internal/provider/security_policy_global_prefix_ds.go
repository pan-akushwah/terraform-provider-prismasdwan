...
// Data source.
var (
	_ datasource.DataSource              = &securityPolicyGlobalPrefixDataSource{}
	_ datasource.DataSourceWithConfigure = &securityPolicyGlobalPrefixDataSource{}
)
...
//	     }
//	  }
func NewSecurityPolicyGlobalPrefixDataSource() datasource.DataSource {
	return &securityPolicyGlobalPrefixDataSource{}
}

type securityPolicyGlobalPrefixDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterSecurityPolicyGlobalPrefix struct {
	...
	Items        []*dsModelSecurityPolicyGlobalPrefixScreenV2N1 `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *securityPolicyGlobalPrefixDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_security_policy_global_prefix"
}

// Schema defines the schema for this data source.
func (d *securityPolicyGlobalPrefixDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *securityPolicyGlobalPrefixDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *securityPolicyGlobalPrefixDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterSecurityPolicyGlobalPrefix
	...
	get_path := "/sdwan/v2.1/api/ngfwsecuritypolicyglobalprefixes/{prefix_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_security_policy_global_prefix", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelSecurityPolicyGlobalPrefixScreenV2N1
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to SecurityPolicyGlobalPrefixScreenV2N1", json_err.Error())
			return
		}

		// lets copy all items into state schema=SecurityPolicyGlobalPrefixScreenV2N1
