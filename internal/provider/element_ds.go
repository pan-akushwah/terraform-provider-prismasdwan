...
type dsModelWithFilterElement struct {
	...
	Items        []*dsModelElementScreenV3N2 `tfsdk:"items"`
}
...
func (d *elementDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterElement
	...
	get_path := "/sdwan/v3.2/api/elements/{element_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_element", (*list_request.ResponseErr).Error())
		}
		return
	}
