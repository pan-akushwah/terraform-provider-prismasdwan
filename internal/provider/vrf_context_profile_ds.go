...
type dsModelWithFilterVrfContextProfile struct {
	...
	Items        []*dsModelVRFContextProfileScreen `tfsdk:"items"`
}
...
func (d *vrfContextProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterVrfContextProfile
	...
	get_path := "/sdwan/v2.0/api/vrfcontextprofiles/{vrf_context_profile_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_vrf_context_profile", (*list_request.ResponseErr).Error())
		}
		return
	}
