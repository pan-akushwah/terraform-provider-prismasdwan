...
func (d *eventCorrelationPolicySetDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterEventCorrelationPolicySet
	...
	get_path := "/sdwan/v2.0/api/eventcorrelationpolicysets/{policy_set_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_event_correlation_policy_set", (*list_request.ResponseErr).Error())
		}
		return
	}
