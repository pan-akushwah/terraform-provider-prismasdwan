...
func (d *siteIotDeviceIdConfigDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterSiteIotDeviceIdConfig
	...
	get_path := "/sdwan/v2.1/api/sites/{site_id}/deviceidconfigs/{config_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_site_iot_device_id_config", (*list_request.ResponseErr).Error())
		}
		return
	}
