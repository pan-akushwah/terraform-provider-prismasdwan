...
// Data source.
var (
	_ datasource.DataSource              = &ipfixLocalPrefixDataSource{}
	_ datasource.DataSourceWithConfigure = &ipfixLocalPrefixDataSource{}
)
...
//	     }
//	  }
func NewIpfixLocalPrefixDataSource() datasource.DataSource {
	return &ipfixLocalPrefixDataSource{}
}

type ipfixLocalPrefixDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterIpfixLocalPrefix struct {
	...
	Items        []*dsModelIPFixLocalPrefixScreen `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *ipfixLocalPrefixDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_ipfix_local_prefix"
}

// Schema defines the schema for this data source.
func (d *ipfixLocalPrefixDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *ipfixLocalPrefixDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ipfixLocalPrefixDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterIpfixLocalPrefix
	...
	get_path := "/sdwan/v2.0/api/ipfixlocalprefixes/{prefix_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_ipfix_local_prefix", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelIPFixLocalPrefixScreen
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to IPFixLocalPrefixScreen", json_err.Error())
			return
		}

		// lets copy all items into state schema=IPFixLocalPrefixScreen
