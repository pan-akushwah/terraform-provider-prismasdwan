...
// Data source.
var (
	_ datasource.DataSource              = &securityPolicySetDataSource{}
	_ datasource.DataSourceWithConfigure = &securityPolicySetDataSource{}
)
...
//	     }
//	  }
func NewSecurityPolicySetDataSource() datasource.DataSource {
	return &securityPolicySetDataSource{}
}

type securityPolicySetDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterSecurityPolicySet struct {
	...
	Items        []*dsModelSecurityPolicyV2SetScreen `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *securityPolicySetDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_security_policy_set"
}

// Schema defines the schema for this data source.
func (d *securityPolicySetDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *securityPolicySetDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *securityPolicySetDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterSecurityPolicySet
	...
	get_path := "/sdwan/v2.0/api/ngfwsecuritypolicysets/{policy_set_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_security_policy_set", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelSecurityPolicyV2SetScreen
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to SecurityPolicyV2SetScreen", json_err.Error())
			return
		}
