...
// Data source.
var (
	_ datasource.DataSource              = &ipfixCollectorContextDataSource{}
	_ datasource.DataSourceWithConfigure = &ipfixCollectorContextDataSource{}
)
...
//	     }
//	  }
func NewIpfixCollectorContextDataSource() datasource.DataSource {
	return &ipfixCollectorContextDataSource{}
}

type ipfixCollectorContextDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterIpfixCollectorContext struct {
	...
	Items        []*dsModelIPFixContextScreen `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *ipfixCollectorContextDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_ipfix_collector_context"
}

// Schema defines the schema for this data source.
func (d *ipfixCollectorContextDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *ipfixCollectorContextDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ipfixCollectorContextDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterIpfixCollectorContext
	...
	get_path := "/sdwan/v2.0/api/ipfixcollectorcontexts/{context_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_ipfix_collector_context", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelIPFixContextScreen
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to IPFixContextScreen", json_err.Error())
			return
		}
