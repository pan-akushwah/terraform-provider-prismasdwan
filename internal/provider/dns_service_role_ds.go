...
// Data source.
var (
	_ datasource.DataSource              = &dnsServiceRoleDataSource{}
	_ datasource.DataSourceWithConfigure = &dnsServiceRoleDataSource{}
)
...
//	     }
//	  }
func NewDnsServiceRoleDataSource() datasource.DataSource {
	return &dnsServiceRoleDataSource{}
}

type dnsServiceRoleDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterDnsServiceRole struct {
	...
	Items        []*dsModelDnsServiceRole `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *dnsServiceRoleDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_dns_service_role"
}

// Schema defines the schema for this data source.
func (d *dnsServiceRoleDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *dnsServiceRoleDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *dnsServiceRoleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterDnsServiceRole
	...
	get_path := "/sdwan/v2.0/api/dnsserviceroles/{dnsservice_role_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_dns_service_role", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelDnsServiceRole
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to DnsServiceRole", json_err.Error())
			return
		}
