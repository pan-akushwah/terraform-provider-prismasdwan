...
type dsModelWithFilterSecurityPolicyStack struct {
	...
	Items        []*dsModelSecurityPolicyV2SetStackScreen `tfsdk:"items"`
}
...
func (d *securityPolicyStackDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterSecurityPolicyStack
	...
	get_path := "/sdwan/v2.0/api/ngfwsecuritypolicysetstacks/{policyset_stack_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_security_policy_stack", (*list_request.ResponseErr).Error())
		}
		return
	}
