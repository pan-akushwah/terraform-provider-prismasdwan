...
// Data source.
var (
	_ datasource.DataSource              = &natLocalPrefixDataSource{}
	_ datasource.DataSourceWithConfigure = &natLocalPrefixDataSource{}
)
...
//	     }
//	  }
func NewNatLocalPrefixDataSource() datasource.DataSource {
	return &natLocalPrefixDataSource{}
}

type natLocalPrefixDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterNatLocalPrefix struct {
	...
	Items        []*dsModelNATLocalPrefix `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *natLocalPrefixDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_nat_local_prefix"
}

// Schema defines the schema for this data source.
func (d *natLocalPrefixDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *natLocalPrefixDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *natLocalPrefixDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterNatLocalPrefix
	...
	get_path := "/sdwan/v2.0/api/natlocalprefixes/{prefix_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_nat_local_prefix", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelNATLocalPrefix
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to NATLocalPrefix", json_err.Error())
			return
		}

		// lets copy all items into state schema=NATLocalPrefix
