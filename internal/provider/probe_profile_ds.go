...
// Data source.
var (
	_ datasource.DataSource              = &probeProfileDataSource{}
	_ datasource.DataSourceWithConfigure = &probeProfileDataSource{}
)
...
//	     }
//	  }
func NewProbeProfileDataSource() datasource.DataSource {
	return &probeProfileDataSource{}
}

type probeProfileDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterProbeProfile struct {
	...
	Items        []*dsModelProbeProfileScreen `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *probeProfileDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_probe_profile"
}

// Schema defines the schema for this data source.
func (d *probeProfileDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *probeProfileDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *probeProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterProbeProfile
	...
	get_path := "/sdwan/v2.0/api/probeprofiles/{profile_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_probe_profile", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelProbeProfileScreen
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to ProbeProfileScreen", json_err.Error())
			return
		}

		// lets copy all items into state schema=ProbeProfileScreen
