...
// Data source.
var (
	_ datasource.DataSource              = &multicastPeerGroupProfileDataSource{}
	_ datasource.DataSourceWithConfigure = &multicastPeerGroupProfileDataSource{}
)
...
//	     }
//	  }
func NewMulticastPeerGroupProfileDataSource() datasource.DataSource {
	return &multicastPeerGroupProfileDataSource{}
}

type multicastPeerGroupProfileDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterMulticastPeerGroupProfile struct {
	...
	Items        []*dsModelMulticastPeerGroupScreenV2N1 `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *multicastPeerGroupProfileDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_multicast_peer_group_profile"
}

// Schema defines the schema for this data source.
func (d *multicastPeerGroupProfileDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
			...
			"items": dsschema.ListNestedAttribute{
				...
				NestedObject: dsschema.NestedAttributeObject{
					Attributes: map[string]dsschema.Attribute{
						...
						"is_source_site_receiver": dsschema.BoolAttribute{
							Required:  false,
							Computed:  false,
							Optional:  true,
							Sensitive: false,
						...
						"peer_sites": dsschema.ListNestedAttribute{
							Required:  false,
							Computed:  false,
							Optional:  true,
							Sensitive: false,
							NestedObject: dsschema.NestedAttributeObject{
								Attributes: map[string]dsschema.Attribute{
									...
									"peer_site_id": dsschema.StringAttribute{
										Required:  false,
										Computed:  false,
										Optional:  true,
										Sensitive: false,
...
func (d *multicastPeerGroupProfileDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *multicastPeerGroupProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterMulticastPeerGroupProfile
	...
	get_path := "/sdwan/v2.1/api/multicastpeergroups/{peer_group_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_multicast_peer_group_profile", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelMulticastPeerGroupScreenV2N1
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to MulticastPeerGroupScreenV2N1", json_err.Error())
			return
		}
		...
		if ans.PeerSites == nil {
			...
			state.PeerSites = []dsModelMulticastPeerSite{}
		} else {
			state.PeerSites = make([]dsModelMulticastPeerSite, 0, len(ans.PeerSites))
