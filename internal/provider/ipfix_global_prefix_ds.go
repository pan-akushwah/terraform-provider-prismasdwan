...
// Data source.
var (
	_ datasource.DataSource              = &ipfixGlobalPrefixDataSource{}
	_ datasource.DataSourceWithConfigure = &ipfixGlobalPrefixDataSource{}
)
...
//	     }
//	  }
func NewIpfixGlobalPrefixDataSource() datasource.DataSource {
	return &ipfixGlobalPrefixDataSource{}
}

type ipfixGlobalPrefixDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterIpfixGlobalPrefix struct {
	...
	Items        []*dsModelIPFixGlobalPrefixScreen `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *ipfixGlobalPrefixDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_ipfix_global_prefix"
}

// Schema defines the schema for this data source.
func (d *ipfixGlobalPrefixDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *ipfixGlobalPrefixDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ipfixGlobalPrefixDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterIpfixGlobalPrefix
	...
	get_path := "/sdwan/v2.0/api/ipfixglobalprefixes/{prefix_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_ipfix_global_prefix", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelIPFixGlobalPrefixScreen
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to IPFixGlobalPrefixScreen", json_err.Error())
			return
		}

		// lets copy all items into state schema=IPFixGlobalPrefixScreen
