...
// Data source.
var (
	_ datasource.DataSource              = &siteSpokeClusterDataSource{}
	_ datasource.DataSourceWithConfigure = &siteSpokeClusterDataSource{}
)
...
//	     }
//	  }
func NewSiteSpokeClusterDataSource() datasource.DataSource {
	return &siteSpokeClusterDataSource{}
}

type siteSpokeClusterDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterSiteSpokeCluster struct {
	...
	Items        []*dsModelSpokeCluster `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *siteSpokeClusterDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (d *siteSpokeClusterDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *siteSpokeClusterDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterSiteSpokeCluster
	...
	get_path := "/sdwan/v2.0/api/sites/{site_id}/spokeclusters/{spoke_cluster_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_site_spoke_cluster", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to SpokeCluster", json_err.Error())
			return
		}

		// lets copy all items into state schema=SpokeCluster
