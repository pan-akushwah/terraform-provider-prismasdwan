...
type dsModelWithFilterSiteNatLocalPrefix struct {
	...
	Items        []*dsModelSiteNATPrefixAssociation `tfsdk:"items"`
}
...
func (d *siteNatLocalPrefixDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterSiteNatLocalPrefix
	...
	get_path := "/sdwan/v2.0/api/sites/{site_id}/natlocalprefixes/{prefix_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_site_nat_local_prefix", (*list_request.ResponseErr).Error())
		}
		return
	}
