...
type dsModelWithFilterWanNetwork struct {
	...
	Items        []*dsModelWANNetworkScreenV2N1 `tfsdk:"items"`
}
...
func (d *wanNetworkDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterWanNetwork
	...
	get_path := "/sdwan/v2.1/api/wannetworks/{wan_network_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_wan_network", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelWANNetworkScreenV2N1
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to WANNetworkScreenV2N1", json_err.Error())
			return
		}

		// lets copy all items into state schema=WANNetworkScreenV2N1
