...
type dsModelWithFilterServiceGroup struct {
...
func (d *serviceGroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var state_with_filter dsModelWithFilterServiceGroup
	...
	get_path := "/sdwan/v2.1/api/servicelabels/{service_label_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_service_group", (*list_request.ResponseErr).Error())
		}
		return
	}
