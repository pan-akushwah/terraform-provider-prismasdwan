...
// Data source.
var (
	_ datasource.DataSource              = &natGlobalPrefixDataSource{}
	_ datasource.DataSourceWithConfigure = &natGlobalPrefixDataSource{}
)
...
//	     }
//	  }
func NewNatGlobalPrefixDataSource() datasource.DataSource {
	return &natGlobalPrefixDataSource{}
}

type natGlobalPrefixDataSource struct {
	client *sdwan.Client
}

type dsModelWithFilterNatGlobalPrefix struct {
	...
	Items        []*dsModelNATGlobalPrefix `tfsdk:"items"`
}

// Metadata returns the data source type name.
func (d *natGlobalPrefixDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "prismasdwan_nat_global_prefix"
}

// Schema defines the schema for this data source.
func (d *natGlobalPrefixDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *natGlobalPrefixDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *natGlobalPrefixDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state_with_filter dsModelWithFilterNatGlobalPrefix
	...
	get_path := "/sdwan/v2.0/api/natglobalprefixes/{prefix_id}"
	...
	if list_request.ResponseErr != nil {
		if IsObjectNotFound(*list_request.ResponseErr) {
			...
			resp.Diagnostics.AddError("error reading prismasdwan_nat_global_prefix", (*list_request.ResponseErr).Error())
		}
		return
	}
	...
	for _, item := range response.Items {
		...
		var state dsModelNATGlobalPrefix
		...
		// if found, exit
		if json_err != nil {
			resp.Diagnostics.AddError("error in json unmarshal to NATGlobalPrefix", json_err.Error())
			return
		}

		// lets copy all items into state schema=NATGlobalPrefix
